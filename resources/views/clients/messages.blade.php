...
(function(){
    ...
    function openCompose(prefill) {
        ...
        container.querySelector('#compose-send').addEventListener('click', async function(){
            ...
            try{
                ...
                const res = await fetch('{{ route('client.messages.send') }}', {
                    method: 'POST',
    ...
    function loadConversation(type, id, name, blocked) {
        ...
        fetch(`{{ url('/messages/conversation') }}/${type}/${id}`, {
    ...
    if (deleteConvBtn) {
        deleteConvBtn.addEventListener('click', function(){
            ...
            if (confirm('Supprimer cette conversation ?')) {
                fetch(`{{ url('/messages/conversation') }}/${currentConversation.type}/${currentConversation.id}`, {
                    method: 'DELETE',
                    headers: {'X-CSRF-TOKEN': csrf, 'X-Requested-With': 'XMLHttpRequest'}
                })
                .then(res => res.json())
    ...
    if (blockBtn) {
        blockBtn.addEventListener('click', function(){
            ...
            if (confirm('Bloquer cette personne ?')) {
                fetch(`{{ url('/messages/block') }}/${currentConversation.type}/${currentConversation.id}`, {
                    method: 'POST',
                    headers: {'X-CSRF-TOKEN': csrf, 'X-Requested-With': 'XMLHttpRequest'}
                })
                .then(res => res.json())
