@php
    ...
    $similar = \App\Models\Produit::where('Categorie', $produit->Categorie)
        ...
        ->map(function($s){
            $im = trim((string)($s->Image ?? ''));
            $u = 'https://via.placeholder.com/120x90?text=No';
            if($im !== ''){
                if(preg_match('/^https?:\/\//i', $im)) $u = $im;
                elseif(\Illuminate\Support\Facades\Storage::exists('public/'.$im)) $u = asset('storage/'.$im);
                elseif(file_exists(public_path($im))) $u = asset($im);
                elseif(file_exists(public_path('images/'.basename($im)))) $u = asset('images/'.basename($im));
            }
            return ['id' => $s->idProduit, 'name' => $s->Nom, 'price' => number_format($s->Prix,0,',',' ') . ' FCFA', 'img' => $u];
        })->toArray();
@endphp
